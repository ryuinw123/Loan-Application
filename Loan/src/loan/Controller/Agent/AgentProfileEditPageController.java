...
import loan.Base.Agent.AgentProfileEditPageBase;
import loan.utils.ConnectionUtil;
import loan.utils.Constants;
import loan.utils.MessageParser;

public class AgentProfileEditPageController extends AgentProfileEditPageBase {
    ...
    {
        if (EAgName.getText().toString().length() > 20)
        {
        lblerror.setText("Name can't have word more than 20");
        }
        else if(EAgPho.getText().toString().length() > 30){
        lblerror.setText("Phone can't have word more than 30");
        }
        else
        {
        ConnectionUtil.EditProfile(EAgName, EAgID, EAgBirth, EAgAdd, EAgPho);
        ConnectionUtil.GetName(AgentNaviBarController.UserText2);
        ConnectionUtil.UserUpdateProfile(EAgName);
       ...
       String s1 = file.toURI().toString().substring(6);       
        try {
            ...
            ConnectionUtil.UserInsertImage();
             AgentNaviBarController.imageView.setImage(image);
